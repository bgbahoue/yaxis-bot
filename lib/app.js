...
const Etherscan     = require('etherscan-api').init(config.etherscan.api_key);
...
function doWork() {
    ...
    .then((block) => {
        ...
        return getInternalTransactionsList(config.yaxis.metavault.contract, block);
    }).then((txnList) => {
        ...
        if (nbTransactions > 0) {
            ...
            log("No new transactions found => going back to sleep for %d ms", config.refresh);
...
function getLatestKnownBlock() {
    return db.getClient()
    .then((client) => {
        return client.query('SELECT timestamp FROM transactions')
        .then((res) => {
            return res.rows.reduce((max, row) => {
                return (row.timestamp > max) ? row.timestamp : max;
            }, 0); // returns 0 if 'res.rows' is empty
        }).catch((err) => {
            error("Error fetching timestamp from DB");
            error(err);
...
function getTokenUSDValue(txn) {
    ...
    .then((client) => {
        return client.query(
            ...
            const price = ((res == null) || !Array.isArray(res.rows)) ? null : res.rows[0].price; // timestamp is a PK so only 1 result possible
            if (price == null) {
