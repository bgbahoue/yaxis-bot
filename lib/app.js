...
function doWork() {
    ...
    .then((block) => {
        ...
        const nextBlock = parseInt(block)+1;
        return getInternalTransactionsList(config.yaxis.metavault.contract, nextBlock); // add 1 to the latest known block to look for new txns
    }).then((txnList) => {
...
function getLatestKnownBlock() {
    ...
    .then((client) => {
        return client.query({
            ...
            }, config.yaxis.metavault.startblock || 0); // returns 0 if 'res.rows' is empty
        }).catch((err) => {
...
function getInternalTransactionsList(contract, startBlock=0, endBlock='latest') {
    return Etherscan.account.tokentx(contract, null, startBlock, endBlock, config.etherscan.page, config.etherscan.offset, 'asc')
        .then((response) => {
            if (
                (response.status == 1) && 
                ((response.message == 'OK') || (response.message == "OK-Missing/Invalid API Key, rate limit of 1/5sec applied")) &&
...
function getTokenUSDValue(txn) {
    ...
    .then((client) => {
        return client.query(
            ...
            if (price == null) {
                log("No price data in DB => getting actual price ...");
...
function scrapTokenUSDValue() {
    var url, queryObj;
    if (config.yaxis.token.token_price.source == "coingecko") {
        url = config.yaxis.token.token_price.api_url;
        queryObj = {
            headers: {
                accept: "application/json"
            },
            params: {
                contract_addresses: config.yaxis.token.contract,
                vs_currencies: 'usd'
            }
        };
    } else {
        throw new Error("Unsupported source for YAXIS token price");
    }

    return axios.get(url, queryObj)
    .then((response) => {
        if (response.status == 200) {
            if (config.yaxis.token.token_price.source == "coingecko") {
                if (
                    response.data[config.yaxis.token.contract] && 
                    response.data[config.yaxis.token.contract].usd &&
                    !Number.isNaN(Number.parseFloat(response.data[config.yaxis.token.contract].usd))
                ) {
                    return Number.parseFloat(response.data[config.yaxis.token.contract].usd);
                }
            } 
        }
        ...
        error(response);
        throw new Error("Invalid response received from %s"+url);
    }).catch((err) => {
...
function publishTxnDetails(txn) {
    ...
    .addFields(
        ...
        { name: "# " + txn.tokenSymbol + " bought back", value: yaxis_emoji + " " + nbTokens.toLocaleString('en', { maximumFractionDigits: 2 }), inline: true},
